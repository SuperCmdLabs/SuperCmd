 ...
 * AI Settings Tab
 *
 * Configure AI provider, API keys, default model, and manage Ollama models.
 */

import React, { useState, useEffect, useCallback } from 'react';
import { Brain, Eye, EyeOff, Download, Trash2, RefreshCw, AlertCircle, ExternalLink } from 'lucide-react';
...
const AITab: React.FC = () => {
  ...
  // Check Ollama status whenever provider is ollama
  useEffect(() => {
    if (!settings || settings.ai.provider !== 'ollama') return;
    refreshOllamaStatus();
  }, [settings?.ai?.provider, settings?.ai?.ollamaBaseUrl, refreshOllamaStatus]);

  // Ollama pull streaming listeners
  useEffect(() => {
  ...
  // Dynamic model list: for Ollama use installed models, for others use static list
          ...
          <div className={`grid gap-4 ${ai.provider === 'ollama' ? 'grid-cols-[minmax(0,58%)_1fr]' : 'grid-cols-1'}`}>
            <div className="space-y-4">
              ...
              <div className="bg-white/[0.03] rounded-lg border border-white/[0.06] p-3">
                ...
                <p className="text-xs text-white/40 mb-3">Used when extensions don't specify a model.</p>
                <select
            ...
            {ai.provider === 'ollama' && (
              <div className="bg-white/[0.03] rounded-lg border border-white/[0.06] p-3 self-start">
